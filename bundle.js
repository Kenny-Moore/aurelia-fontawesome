...
function getPropertyInfo(expression, source) {
    ...
    while (expression instanceof aureliaBinding_60 || expression instanceof aureliaBinding_59) {
        expression = expression.expression;
    }
    let object;
    let propertyName;
    if (expression instanceof aureliaBinding_55) {
        object = aureliaBinding_71(expression.name, source, expression.ancestor);
        propertyName = expression.name;
    }
    else if (expression instanceof aureliaBinding_54) {
        object = getObject(originalExpression, expression.object, source);
        propertyName = expression.name;
    }
    else if (expression instanceof aureliaBinding_53) {
        object = getObject(originalExpression, expression.object, source);
...
class PropertyAccessorParser {
    ...
    parse(property) {
        ...
        if (accessor instanceof aureliaBinding_55
            || accessor instanceof aureliaBinding_54 && accessor.object instanceof aureliaBinding_55) {
            return accessor.name;
        }
        throw new Error(`Invalid property expression: "${accessor}"`);
    }
}
PropertyAccessorParser.inject = [aureliaBinding_37];
...
ValidationErrorsCustomAttribute.inject = [aureliaPal_6.Element, aureliaDependencyInjection_13.of(ValidationController)];
__decorate([
    aureliaTemplating_52({ defaultBindingMode: aureliaBinding_41.oneWay })
], ValidationErrorsCustomAttribute.prototype, "controller", void 0);
__decorate([
    aureliaTemplating_52({ primaryProperty: true, defaultBindingMode: aureliaBinding_41.twoWay })
], ValidationErrorsCustomAttribute.prototype, "errors", void 0);
ValidationErrorsCustomAttribute = __decorate([
    aureliaTemplating_50('validation-errors')
